...
class APIServiceTest(unittest.TestCase):
    ...
    def testExecuteRequestsList(self):
        ...
        service.executeRequestsList(requests)
        for req in requests:
            self.assertIsInstance(req.response, model.Image)

    def testMultithreadingExecuteRequestsList(self):
        credPath = currentDir + "/clientInfo.txt"
        service = request.APIService(credPath)
        requests = [request.ImageRequest(self._clientId, self._clientSecret, "uTHY8_SvFPOXr16D5oAAUg") for i in
                    range(100)]
        service.multithreadingExecuteRequestsList(requests, 5)
        for req in requests:
            self.assertIsInstance(req.response, model.Image)
